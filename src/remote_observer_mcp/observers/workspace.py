...
def _absolute(workspace: WorkspaceConfig, relative: str) -> str:
    ...
    return f"{workspace.root.rstrip('/')}/{relative}"
...
async def workspace_find(
    ...
    if pattern is not None and (
        not _GLOB_RE.fullmatch(pattern) or any(ch in pattern for ch in (";", "`", "$", "|", "&"))
    ):
    ...
    for line in result.stdout.splitlines():
        ...
        if len(paths) >= limit:
            break
    return {"backend": backend.variant, "paths": paths, "truncated": len(paths) >= limit or result.truncated}
...
async def workspace_search(
    ...
    if not isinstance(query, str) or not query or len(query) > 512 or any(ord(ch) < 32 for ch in query):
    ...
    for line in result.stdout.splitlines():
        ...
        if len(matches) >= limit:
            break
    return {"backend": backend.variant, "matches": matches, "truncated": len(matches) >= limit or result.truncated}
...
async def workspace_tree(
    ...
    for line in result.stdout.splitlines():
        ...
        if len(rows) >= 500:
            break
    return {"backend": backend.variant, "lines": rows, "truncated": len(rows) >= 500 or result.truncated}
...
async def file_info(
    ...
    stat_result = await transport.run(
        CommandSpec(argv=("stat", "-c", "%s\t%Y\t%F", absolute))
    )
    ...
    return {"relative_path": relative, "size_bytes": int(fields[0]), "mtime_epoch": int(fields[1]), "type": fields[2]}
...
async def checksum(
    ...
    return {"relative_path": relative, "algorithm": algorithm, "digest": result.stdout.split()[0]}


def register_tools(server: FastMCP, config: AppConfig) -> None:
    async def workspace_call(tool: str, workspace_id: str, operation: Any) -> dict[str, Any]:
    ...
    async def list_workspaces_tool() -> dict[str, Any]:
        ...
        return await run_observed_tool(tool="list_workspaces", host_id=None, resource_id=None, operation=operation)

    @server.tool(name="workspace_find", annotations=_READ_ONLY, structured_output=True)
    async def workspace_find_tool(workspace: str, pattern: str | None = None, kind: str = "all", max_results: int = 100) -> dict[str, Any]:
        async def operation() -> Any:
            item = config.workspace(workspace)
            return await workspace_find(transport_for_host(config.host(item.host_id)), item, pattern, kind, max_results)
    ...
    async def workspace_search_tool(workspace: str, query: str, glob: str | None = None, max_results: int = 100) -> dict[str, Any]:
        async def operation() -> Any:
            item = config.workspace(workspace)
            return await workspace_search(transport_for_host(config.host(item.host_id)), item, query, glob, max_results)
    ...
    async def workspace_read_tool(workspace: str, relative_path: str, start_line: int = 1, end_line: int | None = None) -> dict[str, Any]:
        async def operation() -> Any:
            item = config.workspace(workspace)
            return await workspace_read(transport_for_host(config.host(item.host_id)), item, relative_path, start_line, end_line)
    ...
    async def workspace_tree_tool(workspace: str, relative_path: str = ".", depth: int = 3) -> dict[str, Any]:
        async def operation() -> Any:
            item = config.workspace(workspace)
            return await workspace_tree(transport_for_host(config.host(item.host_id)), item, relative_path, depth)
    ...
    async def file_info_tool(workspace: str, relative_path: str) -> dict[str, Any]:
        async def operation() -> Any:
            item = config.workspace(workspace)
            return await file_info(transport_for_host(config.host(item.host_id)), item, relative_path)
    ...
    async def checksum_tool(workspace: str, relative_path: str) -> dict[str, Any]:
        async def operation() -> Any:
            item = config.workspace(workspace)
            return await checksum(transport_for_host(config.host(item.host_id)), item, relative_path)
